...
function follow(point: Point, center: Point): Point {
  ...
  if (center[0] === point[0]) {
    if (center[1] > point[1]) {
      return movePoint("U", point);
    }
    return movePoint("D", point);
  }
  if (center[1] === point[1]) {
    if (center[0] > point[0]) {
      return movePoint("R", point);
    }
    return movePoint("L", point);
  }
  if (center[0] > point[0]) {
    if (center[1] > point[1]) {
      return movePoint("U", movePoint("R", point));
    }
    return movePoint("D", movePoint("R", point));
  }

  if (center[1] > point[1]) {
    return movePoint("U", movePoint("L", point));
  }
...
function displayVisited(points: Point[]) {
  ...
  for (let y = maxY; y >= minY; y--) {
    ...
    for (let x = minX; x <= maxX; x++) {
      if (x === 0 && y === 0) {
        line.push("s");
      } else if (points.some((p) => x === p[0] && y === p[1])) {
        line.push("#");
      } else {
        line.push(".");
      }
  ...
  for (const line of lines) {
    ...
    while (amount > 0) {
      for (let ix = 0; ix < rope.length; ix++) {
        if (ix === 0) {
          rope[ix] = movePoint(dir as keyof typeof MOVE, rope[ix]);
        } else {
          rope[ix] = follow(rope[ix], rope[ix - 1]);
        }
      }
