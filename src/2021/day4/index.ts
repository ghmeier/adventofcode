...
class Board {
    ...
    mark(key: string) {
        ...
        this.score = Object.values(this.squares).reduce((acc, square) => {
            if (!square.marked) return acc;
            const xKey = `x:${square.x}`
            const yKey = `y:${square.y}`
            if (!acc[xKey]) acc[xKey] = 0
            acc[xKey] += 1
            if (!acc[yKey]) acc[yKey] = 0
            acc[yKey] += 1
            return acc
...
async function fill(file: string) {
    ...
    await handleLines(file, (l) => {
        ...
        if (newBoard) boards.push(new Board());

        const board = boards[boards.length - 1];
        l.trim().split(/\s+/g).forEach((key, x) => board.addSquare(key, x))
    });
...
async function problemOne() {
    ...
    for (let i = 0; i < draw.length; i++) {
        ...
        if (winner) {
            const unmarked = Object.entries(winner.squares).map(([v, square]) => square.marked ? 0 : Number.parseInt(v, 10))
