...
async function problemTwo() {
	...
	for (let y = 0; y < grid.length * 5; y++) {
		...
		for (let x = 0; x < grid[0].length * 5; x++) {
			let risk = -1;
			if (y < grid.length && x < grid[0].length) risk = grid[y][x];
			else if (x - grid[0].length >= 0) risk = row[x - grid[0].length] + 1;
	...
	while (heap.size()) {
		...
		for (const dir of DIRS) {
			...
			const nextKey = ps(next);
			if (!validCell(expanded, next) || visited.has(nextKey)) continue;
