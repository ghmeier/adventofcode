...
async function problemOne() {
  ...
  while (heap.size()) {
    ...
    for (const dir of DIRS) {
      ...
      if (!validCell(grid, next) || visited.has(nextKey)) {
        continue;
      }
      const cost = costs[node.p[1]][node.p[0]] + grid[next[1]][next[0]];
      if (cost >= costs[next[1]][next[0]]) {
        continue;
      }
...
async function problemTwo() {
  ...
  for (let y = 0; y < grid.length * 5; y++) {
    ...
    for (let x = 0; x < grid[0].length * 5; x++) {
      ...
      if (y < grid.length && x < grid[0].length) {
        risk = grid[y][x];
      } else if (x - grid[0].length >= 0) {
        risk = row[x - grid[0].length] + 1;
      } else if (y - grid.length >= 0) {
        risk = expanded[y - grid.length][x] + 1;
      }
      if (risk >= 10) {
        risk = 1;
      }
  ...
  while (heap.size()) {
    ...
    for (const dir of DIRS) {
      ...
      const nextKey = ps(next);
      if (!validCell(expanded, next) || visited.has(nextKey)) {
        continue;
      }
      const cost = costs[node.p[1]][node.p[0]] + expanded[next[1]][next[0]];
      if (cost >= costs[next[1]][next[0]]) {
        continue;
      }
      costs[next[1]][next[0]] = cost;
