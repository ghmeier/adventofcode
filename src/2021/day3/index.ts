...
function partitionSignificant(data: string[][], ix: number) {
    const [zero, one] = data.reduce((acc, d) => {
        if (d[ix] === '1') {
            acc[1].push(d)
        } else {
            acc[0].push(d)
...
function partitionInsignificant(data: string[][], ix: number) {
    ...
    if (one.length >= zero.length) return zero;
    return one
}
...
async function problemTwo() {
    ...
    let co2 = data
    for (let i = 0; i < data[0].length; i++) {
        if (oxygen.length > 1) oxygen = partitionSignificant(oxygen, i)
        if (co2.length > 1) co2 = partitionInsignificant(co2, i)
    }
