...
async function problemOne() {
  ...
  await handleLines(DATA_PATH, (l) => {
    if (l.startsWith("Register A:")) {
      registers.A = BigInt(l.replace("Register A: ", ""));
    } else if (l.startsWith("Register B:")) {
      registers.B = BigInt(l.replace("Register B: ", ""));
    } else if (l.startsWith("Register C:")) {
      registers.C = BigInt(l.replace("Register C: ", ""));
    } else {
      program.push(
        ...l
          .replace("Program: ", "")
  ...
  while (ptr < program.length) {
    ptr = OPERATIONS[Number(program[ptr])](
      registers,
      program[ptr + 1],
      ptr,
      out
...
async function problemTwo() {
  ...
  while (true) {
    ...
    while (ptr < program.length) {
      ...
      if (out.length && out[out.length - 1] !== program[out.length - 1]) {
        break;
      }
      // console.log(r)
    }
    if (out.length === program.length && search === out.join(",")) {
      break;
    }
    steps++;
  }
