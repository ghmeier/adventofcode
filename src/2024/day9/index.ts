...
function mapReallocate(
  ...
  if (!heapMap[f.locations.length]) {
    heapMap[f.locations.length] = new MinHeap();
  }
  ...
  if (next.length < f.locations.length) {
    throw new Error("INVALID");
  }

  f.locations = next.splice(0, f.locations.length);
  if (next.length) {
    heapMap[next.length].set(next[0].toString(), next[0], next);
  }
  return;
}
