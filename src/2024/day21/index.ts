...
function kPaths(s: PadMove, d: PadMove): KeyMove[][] {
  ...
  const moves: KeyMove[] = [];
  if (sx > dx) {
    moves.push(KeyMove.LEFT);
  }
  if (sx < dx) {
    moves.push(KeyMove.RIGHT);
  }
  if (sy > dy) {
    moves.push(KeyMove.UP);
  }
  if (sy < dy) {
    moves.push(KeyMove.DOWN);
  }
