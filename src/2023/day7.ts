...
function matchForCounts(counted: Record<string, number>) {
	...
	if (groups.length === 1) return Matches.FIVE_OF_KIND;
	if (groups.includes(4)) return Matches.FOUR_OF_KIND;
	if (groups.includes(3)) {
		if (groups.length === 2) return Matches.FULL_HOUSE;
		return Matches.THREE_OF_KIND;
	}
	const pairs = groups.filter((v) => v === 2);
	if (pairs.length === 2) return Matches.TWO_PAIR;
	if (pairs.length === 1) return Matches.PAIR;
	return Matches.HIGH;
}
...
function resolveMatchWild(hand: Hand) {
	...
	if (Object.values(counted).length === 1) return Matches.FIVE_OF_KIND;
	const baseMatch = matchForCounts(counted);
	if (!counted.J) return baseMatch;
	let wilds = 0;
	...
	if (baseMatch === Matches.THREE_OF_KIND) {
		if (wilds === 2) return Matches.FIVE_OF_KIND;
		return Matches.FOUR_OF_KIND;
	}
	if (baseMatch === Matches.TWO_PAIR) {
		if (wilds === 2) return Matches.FOUR_OF_KIND;
		return Matches.FULL_HOUSE;
	}
	if (baseMatch === Matches.PAIR) return Matches.THREE_OF_KIND;
	return Matches.PAIR;
}
